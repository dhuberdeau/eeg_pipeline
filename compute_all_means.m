% compute_all_means.m
%
% Wrapper script for the main erp analysis portion of the main pipeline.
% This script computes the raw erp signal (filtered) and then computes the
...
data_directory_input = '/home/david/sandbox_6_redo'; % for milgram (Greg's lab)
data_directory_output = '/home/david/sandbox_6_redo/group_data';
...
[cue_means, cue_sd, cue_err] = compute_mean_erp_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_cue, sub_prefix, erp_file_prefix_precue_hipp, sub_ROI_chs_hipp);

[z_means_cue, z_sd_cue, z_time_cue, z_err_cue] = compute_mean_erp_zscore_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_cue, sub_prefix, z_file_prefix_precue_hipp, sub_ROI_chs_hipp);
...
save z_stats_cue_hipp z_mag_cue_hipp z_diff_cue_hipp
%% Hippocampus ROI for target epoch:
[targ_means, targ_sd, targ_err] = compute_mean_erp_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_targ, sub_prefix, erp_file_prefix_target_hipp, sub_ROI_chs_hipp);

[z_means_targ, z_sd_targ, z_time_targ, z_err_targ] = compute_mean_erp_zscore_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_targ, sub_prefix, z_file_prefix_target_hipp, sub_ROI_chs_hipp);
...
save z_stats_targ_hipp z_mag_targ_hipp z_diff_targ_hipp

%% Hippocampus ROI for movement epoch:
[targ_means, targ_sd, targ_err] = compute_mean_erp_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_move, sub_prefix, erp_file_prefix_movement_hipp, sub_ROI_chs_hipp);

[z_means_targ, z_sd_targ, z_time_targ, z_err_targ] = compute_mean_erp_zscore_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_move, sub_prefix, z_file_prefix_movement_hipp, sub_ROI_chs_hipp);
...
save z_stats_move_hipp z_mag_move_hipp z_diff_move_hipp


%% Hippocampus ROI for ready epoch:
[targ_means, targ_sd, targ_err] = compute_mean_erp_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_ready, sub_prefix, erp_file_prefix_movement_hipp, sub_ROI_chs_hipp);

[z_means_targ, z_sd_targ, z_time_targ, z_err_targ] = compute_mean_erp_zscore_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_ready, sub_prefix, z_file_prefix_ready_hipp, sub_ROI_chs_hipp);
...
save z_stats_ready_hipp z_mag_ready_hipp z_diff_ready_hipp


%% Cortical ROI for pre-cue epoch:
[cue_means, cue_sd, cue_err] = compute_mean_erp_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_cue, sub_prefix, erp_file_prefix_precue_cort, sub_ROI_chs_cort);

[z_means_cue, z_sd_cue, z_time_cue, z_err_cue] = compute_mean_erp_zscore_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_cue, sub_prefix, z_file_prefix_precue_cort, sub_ROI_chs_cort);
...
save z_stats_cue_cort z_mag_cue_cort z_diff_cue_cort
%% Cortical ROI for target epoch:
[targ_means, targ_sd, targ_err] = compute_mean_erp_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_targ, sub_prefix, erp_file_prefix_target_cort, sub_ROI_chs_cort);

[z_means_targ, z_sd_targ, z_time_targ, z_err_targ] = compute_mean_erp_zscore_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_targ, sub_prefix, z_file_prefix_target_cort, sub_ROI_chs_cort);
...
save z_stats_targ_cort z_mag_targ_cort z_diff_targ_cort

%% Cortical ROI for movement epoch:
[targ_means, targ_sd, targ_err] = compute_mean_erp_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_move, sub_prefix, erp_file_prefix_movement_cort, sub_ROI_chs_cort);

[z_means_targ, z_sd_targ, z_time_targ, z_err_targ] = compute_mean_erp_zscore_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_move, sub_prefix, z_file_prefix_movement_cort, sub_ROI_chs_cort);

[z_mag_targ_cort, z_diff_targ_cort] = analyze_roi_erp_zscore(data_directory_output,...
    sub_prefix, z_file_prefix_movement_cort, sub_ROI_chs_cort, sub_chs_to_average_cort);
save z_stats_move_cort z_mag_targ_cort z_diff_targ_cort


%% Cortical ROI for ready epoch:
[targ_means, targ_sd, targ_err] = compute_mean_erp_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_ready, sub_prefix, erp_file_prefix_ready_cort, sub_ROI_chs_cort);

[z_means_targ, z_sd_targ, z_time_targ, z_err_targ] = compute_mean_erp_zscore_all(...
    data_directory_input, data_directory_output,...
    cond_suffix_ready, sub_prefix, z_file_prefix_ready_cort, sub_ROI_chs_cort);
...
save z_stats_ready_cort z_mag_ready_cort z_diff_ready_cort