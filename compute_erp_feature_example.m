function [signals, feature, varargout] = compute_erp_feature_example(input_file, time_window, channel_example)
% feature = compute_erp_feature_example(input_file, time_window)
%
% Function to compute a single example of the computation steps to compute
% the ERP feature (a z-score, based on the baseline window
...
%   (3) channel_example  - the channel to do the example on.
%
...
try
  ...
  signals_for_feature = nan(size(data_z,1), sum(timewind_inds));
  i_ch = channel_example;
  for i_tr = 1:size(data_z,1)

    % extract out the given trial and channel:
    data__ = data_z(i_tr, i_ch, timewind_inds);
    data_ = reshape(data__, size(data__,3), 1);

    % collect signal over which feature will be computed:
    signals_for_feature(i_tr, :) = data_;

    % extract feature:
    feature(i_tr, i_ch) = nanmean(data_);
  end

  signals = signals_for_feature;
